local Diminish = select(2, ...).Diminish
...
local unitEvents = {
    ...
    player = "COMBAT_LOG_EVENT_UNFILTERED", -- dummy value to ensure loop is ran for 'player'
    nameplate = "NAME_PLATE_UNIT_ADDED, NAME_PLATE_UNIT_REMOVED",
}

function Diminish:ToggleUnitEvents(events, enable)
    for event in gmatch(events or "", "([^, %s]+)") do -- csv loop
        if enable then
            if not self:IsEventRegistered(event) then
                self:RegisterEvent(event)
                self:Debug("Registered %s for instance %s.", event, self.currInstanceType)
            end
        else
            if self:IsEventRegistered(event) then
                self:UnregisterEvent(event)
                self:Debug("Unregistered %s for instance %s.", event, self.currInstanceType)
            end
        end
    end
end
        ...
        if settings.enabled then
            ...
            for zone, state in pairs(settings.zones) do
                if state and zone == self.currInstanceType then
                    ...
                    self:ToggleUnitEvents(events, true)
                    break
                else
                    settings.isEnabledForZone = false
                    self:ToggleUnitEvents(events, false)
                end
            end
...
function Diminish:CVAR_UPDATE(name, value)
    if name == "USE_RAID_STYLE_PARTY_FRAMES" then
        Icons:AnchorPartyFrames(value ~= "0")
    end
end
